// lib/features/ai_assistant/services/gemini_service.dart
...
class GeminiService {
  static const _apiKey = 'AIzaSyBat6mhtGOZOMaKR_IXK0_EFlhHLA0dfX8';
...
//     'gemini-2.5-pro',
  ...
  /// 
  /// KEY FIX: uses [message.apiContent] (not [message.content]) so that
  /// file contents are actually sent to Gemini, even though the bubble
  /// only shows a short display text like "📎 cours.txt".
  static Future<String> sendMessage(List<ChatMessage> history) async {
    if (_apiKey.isEmpty) {
      return '⚠️ Clé API manquante.\nLance avec : flutter run --dart-define=GEMINI_API_KEY=ta_clé';
    }

    try {
      // ✅ Use apiContent — this carries the real file text when a file was attached
      final geminiHistory = history
          .where((m) => !m.isLoading)
          .map((m) => Content(
                m.role == MessageRole.user ? 'user' : 'model',
                [TextPart(m.apiContent)],  // ← apiContent, NOT content
              ))
          .toList();

      final previousMessages =
          geminiHistory.sublist(0, geminiHistory.length - 1);
      final lastApiContent = history.last.apiContent; // ← apiContent here too
