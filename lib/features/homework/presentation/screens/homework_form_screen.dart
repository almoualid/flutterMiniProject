...
class HomeworkFormScreen extends StatefulWidget {
  ...
  const HomeworkFormScreen({super.key, this.homework});
  ...
  Future<void> _saveHomework() async {
    ...
    if (_selectedCourseId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Veuillez sélectionner un cours'), behavior: SnackBarBehavior.floating),
      );
      return;
    }
    ...
    try {
      ...
      if (widget.homework == null) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Devoir "${hw.title}" ajouté'), behavior: SnackBarBehavior.floating),
        );
      } else {
        await _homeworkRepository.update(hw);
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Devoir "${hw.title}" mis à jour'), behavior: SnackBarBehavior.floating),
        );
      }
            ...
            children: [
              ...
              StreamBuilder<List<Course>>(
                ...
                builder: (context, snapshot) {
                  ...
                  return DropdownButtonFormField<String>(
                    value: _selectedCourseId.isEmpty ? null : (courses.any((c) => c.id == _selectedCourseId) ? _selectedCourseId : null),
                    decoration: InputDecoration(
                      labelText: 'Cours',
              ...
              Card(
                ...
                child: SwitchListTile(
                  title: const Text('Marquer comme fait', style: TextStyle(fontWeight: FontWeight.w600)),
